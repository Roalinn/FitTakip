...
export default function ExerciseProgress() {
    ...
    // Extract all unique exercise names from gym logs, separated by type
    const { weightNames, cardioNames, exerciseNames } = useMemo(() => {
        ...
        state.gymLog.forEach(log => {
            ...
            log.exercises.forEach(ex => {
                if (ex.name && ex.name.trim()) {
                    if (isCardio) cNames.add(ex.name.trim());
                    else wNames.add(ex.name.trim());
                }
            });
        });
        const wArr = [...wNames].sort();
        const cArr = [...cNames].sort();
        return {
            weightNames: wArr,
            cardioNames: cArr,
            exerciseNames: [...wArr, ...cArr],
        };
    }, [state.gymLog]);
        ...
        <div className="space-y-4">
            ...
            <div className="space-y-2">
                ...
                {cardioNames.length > 0 && (
                    <div className="flex flex-wrap gap-2">
                        {cardioNames.map(name => (
                            <button
                                key={name}
                                className={`btn btn-sm rounded-xl font-medium transition-all duration-200 ${selectedExercise === name ? 'btn-secondary' : 'btn-ghost bg-base-200'}`}
                                onClick={() => setSelectedExercise(name)}
                            >
                                {name}
                            </button>
